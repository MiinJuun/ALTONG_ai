...
4단계에서는 합성 사례 17건을 추가해 현재 51건이다. 분포와 모순 검사는
`filtering_training.audit_dataset`으로 점검했다. 현재 분할은 파이프라인
점검용이므로, 본학습 전에 유사 사례가 섞이지 않은 충분한 평가셋을
고정해야 한다. 34건 분할의 이전 기준 평가는 새 51건 분할과 직접 비교할 수
없으므로 본학습 전에 기준 평가를 다시 실행한다. 학습 산출물은
`filtering_training/outputs`에 두고 Git에서
제외한다.
